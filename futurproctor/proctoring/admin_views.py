...
def publish_exam(request, exam_id):
    ...
    was_already_published = exam_paper.published
    exam_paper.published = True
    exam_paper.save()
    
      # Send email notifications to all approved students (only if newly published)
    if not was_already_published:
        try:
            from .email_service import send_exam_published_bulk
            email_stats = send_exam_published_bulk(exam_paper)
            
            if email_stats['success_count'] > 0:
                messages.success(
                    request, 
                    f"Exam '{exam_paper.title}' has been published! "
                    f"Email notifications sent to {email_stats['success_count']}/{email_stats['total_count']} students."
                )
            else:
                messages.success(
                    request, 
                    f"Exam '{exam_paper.title}' has been published! (Email notifications failed)"
                )
        except Exception as e:
            logger.error(f"Failed to send exam publish notifications: {str(e)}")
            messages.success(request, f"Exam '{exam_paper.title}' has been published! (Email service unavailable)")
    else:
        messages.info(request, f"Exam '{exam_paper.title}' was already published.")
    return redirect('exam_paper_detail', exam_id=exam_id)
